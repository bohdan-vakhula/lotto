...
if (isset($_POST) && ($_POST['Email'] != '')) {
	...
	if (!is_array($result)) {
		...
		$result = json_decode(apiCall("userinfo/get-personal-details-by-email", $data), true);
	}
	if (is_array($result) && $result['MemberId'] !== "") {
		...
		if (is_array($result) && $result['ProductManagementCounter'] !== "") {
			...
			$script  = '<script>document.location.href="https://www.orolotto.com/en/autologin/' . $_SESSION['user_data']['Result']['UserSessionId'] . '/' . BRAND_ID . '/' . $_SESSION['utm_campaign'] . '";</script>';
		}
		else {
			$script  = '<script>document.getElementById("errorfromapi").innerHTML ="Email address is already registered";</script>';
		}
	}	
}
