jQuery(document).ready(function($) {
    ...
    jQuery("#single_continue").click(function(e) {
        ...
        //debugger;
        getProductDiscount(data.productId, data.draws, data.lotteryId, data.subscription);
        ...
        if (lines >= minl && lines <= maxl) {
            ...
            jQuery("#errorbox").html("Ticket lines must between " + minl + " and " + maxl);
    ...
    if(CART_CONFIG.CART_PRODUCTS){
        ...
        scope.$apply(function() {
            ...
            for (var i = 0; i < lengthArr; i++) {
                if (i + 1 === lengthArr) {
                    scope.saveToCartPredefined(parsedProducts[i], parsedProductsHash, true);
                } else {
                    scope.saveToCartPredefined(parsedProducts[i], parsedProductsHash, false);
                }
            }
        });
    }
...
function buyNowProductBtn(productId, draws, lotteryType, qty) {
    ...
    if (productId === 999) {
        ...
        var itemToSave = {
            ...
            Draws: draws,
            Price: calculatePriceAndDiscountByDraws(productId, draws, lotteryType).price,
            Qty: qty
        };
...
function getProductDiscount(productId, draws, LotteryTypeId, subscription) {
    ...
    if (typeof lotteries_rule !== "undefined" || lotteries_rule !== null) {
        ...
        if (lotteries_rule_parsed == null) {
            ...
            alert("Your device is not fully supported! Please turn off Private/Incognito browsing.");
        }
...
function calculatePriceAndDiscountByDraws(productId, draws, lotteryTypeId) {
    ...
    if (productId == 999) {
        ...
        var firstDiscount  = (firstPrice/(1-getProductDiscount(3, draws, 1))-firstPrice).toFixed(2);
        var secondDiscount = (secondPrice/(1-getProductDiscount(3, draws, 14))-secondPrice ).toFixed(2);
        var thirdDiscount  = (thirdPrice/(1-getProductDiscount(3, draws, 2))-thirdPrice ).toFixed(2);

        totalPrice = (firstPrice + secondPrice + thirdPrice).toFixed(2);
        discount   = (parseFloat(firstDiscount) + parseFloat(secondDiscount) + parseFloat(thirdDiscount)).toFixed(2);
    } else if (productId == 998) {
        var firstPrice     = getProductPrices(3, draws, 5).Price;
        var secondPrice    = getProductPrices(3, draws, 9).Price;
        var firstDiscount  = (firstPrice/(1-getProductDiscount(3, draws, 5))-firstPrice).toFixed(2);
        var secondDiscount = (secondPrice/(1-getProductDiscount(3, draws, 9))-secondPrice).toFixed(2);

        totalPrice = (firstPrice + secondPrice).toFixed(2);
        discount   = (parseFloat(firstDiscount) + parseFloat(secondDiscount)).toFixed(2);
    } else {
        price = parseFloat(getProductPrices(productId, draws, lotteryTypeId).Price);
        if (productId == 14) {
            discount = (0).toFixed(2);
        } else {
            discount = (price/(1-getProductDiscount(productId, draws, lotteryTypeId)) - price).toFixed(2);
        }
...
function viewMorePopUp(productId, draws, lotteryTypeId) {
    ...
    if (productId == 999) {
        ...
        $('#popUpText').text('Win the biggest lottery jackpot in the world! Boost your winning odds by 5000%. With our Groups you get more tickets, win more and pay less.');
        $('#popUpProduct').html('<img src="' + CONFIG.templateURL + '/images/product-logo-america.png" alt="" />');
    } else if (productId == 998) {
        $('#popUpTitle').text('Euro Group');
        $('#popUpText').text('Play it European style, with 3 of the biggest lotteries in Europe. All lucky-star and Euro number combinations are covered, making you a winner on every draw.');
        $('#popUpProduct').html('<img src="' + CONFIG.templateURL + '/images/product-logo-euro.png" alt="" />');
    } else if (productId == 14) {
        $('#popUpTitle').text('Jackpot Hunter');
        $('#popUpText').text('Never miss a BIG Jackpot! With the ‘Jackpot Hunter’ you will always play the biggest Jackpot at any given time with at least 100 tickets!');
        $('#popUpProduct').html('<img src="' + CONFIG.templateURL + '/images/product-logo-tophunter.png" alt="" />');
    } else {
        if (lotteryTypeId == 1) {
            $('#popUpTitle').text('Powerball');
            $('#popUpText').text('Win the biggest lottery jackpot in the world! Boost your winning odds by 5000%. With our Groups you get more tickets, win more and pay less.');
            $('#popUpProduct').html('<div class="lottinner sliderlogoPowerBall"></div>');
        }
        if (lotteryTypeId == 2) {
            $('#popUpTitle').text('MegaMillions');
            $('#popUpText').text('It’s a game of quantity. Get more tickets and enjoy higher winning odds.');
            $('#popUpProduct').html('<div class="lottinner sliderlogoMegaMillions"></div>');
        }
        if (lotteryTypeId == 3) {
            $('#popUpTitle').text('Lotto649');
            $('#popUpText').text('It’s a game of quantity. Get more tickets and enjoy higher winning odds.');
            $('#popUpProduct').html('<div class="lottinner sliderlogoLotto649"></div>');
        }
        if (lotteryTypeId == 4) {
            $('#popUpTitle').text('LaPrimitiva');
            $('#popUpText').text('It’s a game of quantity. Get more tickets and enjoy higher winning odds.');
            $('#popUpProduct').html('<div class="lottinner sliderlogoLaPrimitiva"></div>');
        }
        if (lotteryTypeId == 5) {
            $('#popUpTitle').text('EuroMillions');
            $('#popUpText').text('It’s a game of quantity. Get more tickets and enjoy higher winning odds.');
            $('#popUpProduct').html('<div class="lottinner sliderlogoEuroMillions"></div>');
        }
        if (lotteryTypeId == 8) {
            $('#popUpTitle').text('SuperEnalotto');
            $('#popUpText').text('Beat the lottery with a Group. Play with hundreds of tickets each draw, increase your winning odds by 5000% and pay only a fraction of the price.');
            $('#popUpProduct').html('<div class="lottinner sliderlogoSuperEnalotto"></div>');
        }
        if (lotteryTypeId == 9) {
            $('#popUpTitle').text('EuroJackpot');
            $('#popUpText').text('It’s a game of quantity. Get more tickets and enjoy higher winning odds.');
            $('#popUpProduct').html('<div class="lottinner sliderlogoEuroJackpot"></div>');
        }
        if (lotteryTypeId == 10) {
            $('#popUpTitle').text('ElGordo');
            $('#popUpText').text('It’s a game of quantity. Get more tickets and enjoy higher winning odds.');
            $('#popUpProduct').html('<div class="lottinner sliderlogoElGordo"></div>');
        }
        if (lotteryTypeId == 11) {
        ...
        if (lotteryTypeId == 12) {
            ...
            $('#popUpText').text('It’s a game of quantity. Get more tickets and enjoy higher winning odds.');
            $('#popUpProduct').html('<div class="lottinner sliderlogoUkLotto"></div>');
        }
        if (lotteryTypeId == 14) {
            $('#popUpTitle').text('NewYorkLotto');
            $('#popUpText').text('It’s a game of quantity. Get more tickets and enjoy higher winning odds.');
            $('#popUpProduct').html('<div class="lottinner sliderlogoNewYorkLotto"></div>');
        }
    }

    if (productId == 14) {
        $('#DrawsSelect').html('<option value="2">2</option>' +
        ...
        $('#DrawsSelect').html('<option value="1">1</option>' +
